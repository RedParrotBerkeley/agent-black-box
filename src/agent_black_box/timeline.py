...
def render_timeline(run: TraceRun, compact: bool = False) -> str:
    ...
    if compact:
        lines.append(f"visible_events: {len(visible_events)}")
    ...
    if compact and len(visible_events) != run.event_count:
        ...
        lines.extend(["", f"omitted_events: {run.event_count - len(visible_events)} ({omitted})"])
...
def _render_value(kind: str, key: str, value: Any, compact: bool = False) -> str:
    if isinstance(value, dict):
        if key == "arguments":
            ...
            for subkey in ["command", "path", "action", "query", "messageId", "to"]:
                if subkey in value:
                    preferred.append(f"{subkey}={_clip(str(value[subkey]), 80 if compact else 140)}")
            if preferred:
                return "{" + ", ".join(preferred) + "}"
            compact_text = ", ".join(f"{k}={_clip(str(v), 60 if compact else 120)}" for k, v in sorted(value.items())[:3])
            return "{" + compact_text + "}"
        if key == "details":
            preferred = []
            ...
            if "message" in value and isinstance(value["message"], dict):
                ...
                content = _clip(str(msg.get("content", "")), 80 if compact else 160)
                parts = []
                if msg_id:
        ...
        if key == "content" and kind == "assistant_thinking":
            ...
            return _clip(str(thinking or "thinking"), 120 if compact else 240)
        compact_text = ", ".join(f"{k}={_clip(str(v), 60 if compact else 120)}" for k, v in sorted(value.items())[:3])
        return "{" + compact_text + "}"
    ...
    return _clip(text, 160 if compact else 320)
